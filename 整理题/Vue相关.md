...
+ 跨平台，本质上是一个 对象结构，不依赖于平台环境，因此可以具有跨平台的能力，例如 RN
...
+ 预处理之后，需要判断哪些节点需要被移动、删除、创建
  ...
  + 接着循环遍历旧节点列表，尝试在新的节点中找到 key 相同的节点，移除没有找到的节点，同时维护更新数组位置信息
  + 接着就是新增和移动的处理，**从尾到头**遍历数组，如果是新增（-1），则新增节点，如果非新增，则表示需要被移动，这时候将其移动至对应的 index （插入到该节点在新节点中位置的下一个节点位置）之前。
...
**根本意义**在于解决了UI与数据状态的同步。使用 jquery 避免不了通过 appendChild 、createElement API 操作 DOM来保证UI层和数据层统一。这样带来的问题也是消耗性能，不利于维护。框架的意义在于自动帮我们做了同步的工作。
