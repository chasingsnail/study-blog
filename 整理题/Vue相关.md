...
+ Keep-alive 使用时配合 include、exclude
+ dev-tool 调试方便
+ 组件递归调用自身时，只能通过 `name` 选项
...
**编译的顺序**是先父后子。父组件完成编译后会生成插槽节点对应的 vnode，数据的作用域是父组件实例。在子组件 init 时，此时父组件已经编译完成，维护了一个 slots 对象按插槽名称 key 获取父组件中对应的编译完成后的 child 节点 Vnode 。在子组件生成 slot 节点时，可以借助这个 slots 对象，拿到已经渲染好的 vnode，实现了在父组替换子组件插槽的内容了。
...
1. 首先初始化生命周期，初始化事件中心，初始化渲染，初始化 data、props、computed、watcher 等等
2. 最后调用 $mount 方法进行挂载
   1. 过程中首先会判断如果没有定义 `render` 方法，则会把 `el` 或者 `template` 字符串转换成 `render` 方法。
   2. 接着会去实例化一个组件的渲染`Watcher`，作用一个是初始化的时候会执行回调函数，另一个是当 vm 实例中的监测的数据发生变化的时候执行回调函数。
   3. 在执行回调的过程中会通过 render 方法，执行 createElement并返回 Vnode 树，这个过程中访问到响应式数据，这时候就会收集到 watcher 。
   4. 最后调用 patch 方法，递归生成子组件，最终通过原生 API 插入真实 dom 中
...
Vue.extend 作用是构造一个 Vue 的子类，通过典型的原型继承的方式把一个对象转换成继承 Vue 的构造子类，并且将传入的对象合并到子类的 options，并且对本身做了另外拓展了一些属性。当去实例化这个构造器的时候，就会执行 this._init 方法，这其实就是 Vue 实例初始化的过程。我们可以配合 $mount 方式，将这个实例的 $el 插入到 dom 中想要的位置。

如果我们不需要通过组件名称的时候，例如通过 this.$message 这样命令的方式调用唤起一个弹窗提示，可以使用 extend，可以传入 options 给构造器。

Vue.Component 主要用于注册或获取全局组件，可以传入一个扩展过的构造器来注册组件。

相当于给 Vue.extend 来创建一个组件，可以通过 Vue.Component 来注册这个组件。

### Composition API vs. Mixin 

mixin 用来抽离组件中的可复用功能，混入对象的选项将被“混合”进入该组件本身的选项。

mixin 对象的结构和书写 Vue 组件 script 部分是相同的，当混入项和组件项冲突时优先取组件项。除了对于组件的混入，mixin 还有全局混入的功能，Vue-Router 以及 Vuex 都是通过全局混入的方式让我们在实例内部通过 this.$store、this.$route 访问到相应的实例。

mixin 问题

+ 变量来源不明确，不利于代码阅读，不太容易看出变量是属于当前组件还是属于若干个 mixin 中的一个
+ 多个 mixin 可能会造成命名冲突，会有覆盖的情况
+ mixins 和组件有着隐含的依赖关系，还可能会出现多对多的关系，复杂度高，维护困难

composition API 的优势，可复用的方法定义在一个 js 模块里面，导出对应的方法。

+ setup 函数开发，在导入复用函数时，需要显式命名任何状态或从组成函数返回的方法，这样明确了变量的来源，也很容易看出是否有命名冲突
+ 隐含依赖关系（方法需要依赖组件的状态）通过必须显式地将响应式数据显式传递给组合函数
...
用于注册插件，自身会调用插件自带的静态 install 方法，在注册时执行。并且在方法内部会设置一个缓存变量，避免插件的重复注册。并且内部会可以获取到 Vue 对象，这样在编写插件的时候不需要额外的 import
