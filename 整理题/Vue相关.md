...
key 作为唯一的标记，可以让整个 diff 操作更快速准确。准确体现在 vnode 对比时可以避免**就地复用**的情况，保证渲染的准确性。例如列表中每一行都包含一个文本输入框，删除中间某一行后，文本库的内容异常。

在 diff 过程中四种匹配方式都没有命中的情况下，会生成以 key 为键值的 map 对象来获取对应的节点，这种方式比不设置 key 遍历节点来的快速。

使用 index 作为 key 在某些情况例如长列表删除的场景下，会让 Vue 复用错节点。即当列表依赖子组件的状态时，改变了列表的顺序，就会出现错误复用的结果。
